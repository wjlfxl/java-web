...
public class attachmentMail {
    public static void main(String[] args) throws GeneralSecurityException, MessagingException {
        ...
        text.setContent("复杂邮件2<img src='cid:bz.jpg'","text/html;charset=UTF-8");
        ...
        attachment.setDataHandler(new DataHandler(new FileDataSource("E:\\仓库\\JavaWEB\\javaweb\\note.md")));
        attachment.setFileName("1111.md");

        MimeBodyPart contentText = new MimeBodyPart();
        contentText.setContent(mmp);

        //拼接附件
        MimeMultipart allFile = new MimeMultipart();
        allFile.addBodyPart(attachment);//附件
        allFile.addBodyPart(contentText);//正文
        allFile.setSubType("mixed");//正文和附件都存在邮件中，类型设置为mixed
        //设置到消息中，保存修改
        message.setContent(allFile);//包编辑好的邮件放到消息中
        message.saveChanges();
